...
def main() -> int:
    request = GatewayGovernanceRequest(
        ...
        correlation_id="demo-gateway-secret-route-block",
        raw_prompt="send api_key=demo-secret to external model",
    )
    result = evaluate_gateway_governance(request, config=GatewayGovernanceConfig(enabled=True))
