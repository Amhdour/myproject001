#!/usr/bin/env python3
"""Fail on unsafe portfolio readiness claims in reviewer-facing docs."""
...
def main() -> int:
    ...
    if violations:
        print("FAIL: unsafe production/enterprise/compliance/live-enforcement claims found.")
        for path, line_number, phrase, line in violations:
            print(f"- {path}:{line_number}: matched {phrase!r}: {line}")
        return 1

    print(
        "PASS: claim-boundary check found no unsafe positive readiness claims "
